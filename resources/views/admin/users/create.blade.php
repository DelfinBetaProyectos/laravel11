<x-dashboard-layout>
    ...
    <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
      ...
      <li aria-current="page">
        <div class="flex items-center">
          ...
          <span class="ms-1 text-sm font-semibold md:ms-2 dark:text-gray-400">{{ __('Add User') }}</span>
        </div>
      </li>
    </ol>
  </nav>
  ...
  <div class="my-6 max-w-7xl mx-auto sm:px-6 lg:px-8">
    <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
      <div class="p-4">
        <h1 class="leading-tight font-semibold text-2xl dark:text-gray-400">{{ __('Add User') }}</h1>
      </div>
        ...
        <form action="{{ route('admin.users.store') }}" method="POST" enctype="multipart/form-data">
          ...
          <div class="my-6 grid grid-cols-1 lg:grid-cols-6 gap-4 lg:gap-6">
            ...
            <div class="lg:col-span-3 relative z-0 w-full group">
              ...
              <x-input id="firstname" type="text" name="firstname" :value="old('firstname')" placeholder=" " class="w-full" required autofocus />
              <x-input-error for="firstname" class="mt-2" />
            </div>
            <!-- Last Name -->
            <div class="lg:col-span-3 relative z-0 w-full group">
              <x-label for="lastname" value="{{ __('Last Name') }}" />
              <x-input id="lastname" type="text" name="lastname" :value="old('lastname')" placeholder=" " class="w-full" required />
              <x-input-error for="lastname" class="mt-2" />
            </div>
            <!-- Email -->
            <div class="lg:col-span-3 relative z-0 w-full group">
              <x-label for="email" value="{{ __('Email') }}" />
              <x-input id="email" type="email" name="email" :value="old('email')" placeholder=" " class="w-full" required />
            ...
            <div class="lg:col-span-3 relative z-0 w-full group">
              ...
              <x-input id="password_confirmation" type="password" name="password_confirmation" class="w-full" placeholder=" " required />
                ...
                <div class="flex flex-wrap">
                  @foreach ($roles as $role)
                    <div class="flex items-center me-4">
                      <input type="checkbox" id="role{{ $role->name }}" name="roles[]" value="{{ $role->name }}" class="w-4 h-4 text-blue-600 bg-gray-100 border-2 border-gray-300 rounded-sm focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 dark:focus:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600" />
